...
# Selectors for the various overlays LinkedIn injects on the feed: the
# messaging-overlay bubble in the bottom-right, the global modal/dialog
# scrim, the "Subscribe to Premium" upsell, the cookie banner, the
# "Open in app" interstitial. Closed best-effort — never raises.
_FEED_POPUP_CLOSE_SELECTORS = (
    # Generic modal / dialog close buttons
    "div[role='dialog'] button[aria-label*='Dismiss' i]",
    "div[role='dialog'] button[aria-label*='Close' i]",
    "div[role='alertdialog'] button[aria-label*='Dismiss' i]",
    "div[role='alertdialog'] button[aria-label*='Close' i]",
    # Generic dismiss buttons LinkedIn uses on feed pop-ins
    "button.artdeco-modal__dismiss",
    "button.artdeco-toast-item__dismiss",
    # Messaging overlay bubble (bottom-right) — collapse so it doesn't cover posts
    ...
    # "Subscribe to Premium" upsell
    "section.premium-upsell-link button[aria-label*='Dismiss' i]",
    "section[class*='premium'] button[aria-label*='Dismiss' i]",
    # "Open in app" interstitial
    "button[aria-label*='Continue on web' i]",
)


def dismiss_feed_popups(page: Page) -> int:
    """Best-effort: close any popups / overlays / scrims covering the feed so the
    scraper sees the underlying posts. Returns the number of close clicks that
    landed. Never raises — every selector miss is swallowed."""
    closed = 0
    # First pass: known selectors.
    for sel in _FEED_POPUP_CLOSE_SELECTORS:
        try:
            ...
            continue
    # Second pass: Escape twice to dismiss any remaining focused modal/scrim
    # (the global keyboard-shortcut bottom sheet, the search dropdown, etc.).
    try:
...
def goto_home_feed(page: Page) -> str:
    ...
    random_sleep(2, 4)
    # Clear any popups (cookie banner, messaging overlay, premium upsell, etc.)
    # before the scraper starts walking the DOM.
    try:
