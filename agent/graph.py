...
POST_BATCH_TARGET = int(os.getenv("POST_BATCH_TARGET", "40"))
POST_BATCH_MAX_SCROLLS = int(os.getenv("POST_BATCH_MAX_SCROLLS", "12"))
# When this many consecutive scroll passes yield no NEW posts, the feed has
# stopped lazy-loading fresh content — reload it to pull a fresh batch.
POST_BATCH_STALL_LIMIT = int(os.getenv("POST_BATCH_STALL_LIMIT", "3"))
# How many times we reload the feed within a single batch to keep harvesting
# once scrolling alone is exhausted. Each reload reshuffles the feed and
# surfaces posts the previous scroll didn't reach.
POST_BATCH_MAX_REFRESHES = int(os.getenv("POST_BATCH_MAX_REFRESHES", "3"))
...
def _resolve_missing_permalinks(page, scraped_posts: list[dict], budget: int) -> int:
    ...
    share dialog. Mutates the scraped dicts in place. `budget` is the number of
    dialog lookups still allowed; returns the remaining budget so callers can
    thread it across scroll passes (each lookup costs ~2-3s of UI interaction).

    Only pass posts you actually intend to keep — resolving permalinks for
    already-seen candidates on every pass is the main scrape-loop time sink."""
    if budget <= 0:
        return 0
    remaining = budget
...
def _scrape_feed_batch(page, search_url: str, target: int) -> list[dict]:
    ...
    dialog_budget = DIALOG_PERMALINK_BUDGET_PER_BATCH
    stall = 0            # consecutive scroll passes that added no new posts
    refreshes_left = POST_BATCH_MAX_REFRESHES
    scroll_i = 0
    pass_no = 0

    while True:
        pass_no += 1
        # Clear any popup / overlay / scrim that may have been injected between
        # scrolls (messaging bubble, "Open in app", premium upsell, etc.) so
        # the JS scraper sees the underlying posts.
        try:
            dismiss_feed_popups(page)
        except Exception:
            pass
        ...
        # Dedup FIRST so we only spend time (and the share-dialog budget) on
        # posts we'll actually keep. Resolving permalinks for already-seen
        # candidates on every pass was the loop's main time sink.
        new_posts: list[dict] = []
        for scraped in scraped_posts:
            content = scraped.get("content") or ""
            if not content:
                continue
            # Prefer post_url permalink, then author_url + normalized body, then
            # normalized body — see _stable_post_id. A naive sha1(content) hash
            # flips on every reload because the JS scraper picks up reaction /
            ...
            if is_processed("posts", post_id):
                ...
                continue
            scraped["_post_id"] = post_id
            new_posts.append(scraped)

        # Backfill permalinks only for the new posts, bounded by a batch-wide
        # budget threaded across passes.
        dialog_budget = _resolve_missing_permalinks(page, new_posts, dialog_budget)

        added = 0
        for scraped in new_posts:
            post_id = scraped.pop("_post_id")
            details = {
                **scraped,
                "identifier": post_id,
                "source_url": page.url,
            ...
            added += 1
            if len(batch) >= target:
                print(f"[POST] pass {pass_no}: +{added} new → reached target {target}", flush=True)
                return batch

        stall = stall + 1 if added == 0 else 0
        print(
            f"[POST] pass {pass_no} (scroll {scroll_i}/{POST_BATCH_MAX_SCROLLS}): "
            f"{len(scraped_posts)} candidates, +{added} new "
            f"(batch={len(batch)}/{target}, stall={stall}, refreshes_left={refreshes_left})",
            flush=True,
        )

        # Refill exhausted: either the scroll budget ran out or the feed stopped
        # surfacing new posts. Reload to reshuffle and keep harvesting, until we
        # run out of refreshes.
        scroll_exhausted = scroll_i >= POST_BATCH_MAX_SCROLLS
        feed_stalled = stall >= POST_BATCH_STALL_LIMIT
        if scroll_exhausted or feed_stalled:
            if refreshes_left <= 0:
                reason = "scroll budget spent" if scroll_exhausted else "feed stalled"
                print(f"[POST] {reason}, no refreshes left — stopping at {len(batch)} posts", flush=True)
                break
            refreshes_left -= 1
            reason = "scroll budget spent" if scroll_exhausted else f"feed stalled ({stall} dry passes)"
            print(f"[POST] {reason} — refreshing feed ({refreshes_left} refreshes left)", flush=True)
            goto_home_feed(page)
            scroll_i = 0
            stall = 0
            continue
...
def _scroll_feed(page) -> None:
    """Scroll the feed down to trigger LinkedIn's intersection-observer lazy
    loader. `page.mouse.wheel` only scrolls the element under the cursor, which
    defaults to (0,0) — the fixed nav bar — so on its own it does nothing
    visible. We move the cursor over the feed first, wheel twice, then back it
    up with a JS window.scrollBy + scrolling the last post into view so the
    scroll lands regardless of which element actually owns the scrollbar."""
    # Mouse-wheel over the centre of the viewport (1280x720 context).
    try:
        ...
        pass
    # JS fallback: scroll the window, and pull the last loaded post into view so
    # the observer that gates lazy-loading actually fires.
    try:
        page.evaluate(
            """() => {
                window.scrollBy(0, 3000);
                const posts = document.querySelectorAll(
                    "div[data-id^='urn:li:activity'], div.feed-shared-update-v2, "
        ...
        pass
    # Keyboard nudge — End scrolls the focused scroll container to its bottom,
    # catching layouts where neither the wheel nor window.scrollBy owns it.
    try:
...
def search_post_node(state: AgentState) -> dict:
    ...
    if queue:
        ...
        print(f"[POST] Yield queued post ({len(queue)} remaining) — author: {author}", flush=True)
        if content:
            print(f"[POST] ─── full post content ───\n{content}\n[POST] ─── end post ───", flush=True)
        return {
    ...
    # Queue empty: navigate to /feed/ and refill by scrolling. We deliberately
    # only scrape the user's home feed here — no search-bar fallback — per
    # the explicit instruction to operate from the feed alone.
    ...
    if not batch:
        streak = int(state.get("empty_streak", 0)) + 1
        lo, hi = _empty_backoff(streak)
        print(f"[POST] No new posts on feed. Sleeping {lo}-{hi}s (streak={streak})...", flush=True)
        random_sleep(lo, hi)
...
def network_node(state: AgentState) -> dict:
    ...
    # 1st-degree → already connected, so the DM is ready to send immediately.
    # We do NOT send it: the bot only drafts DMs. The draft is queued as
    # 'accepted' (= ready to DM) so it shows up under "Ready to DM" in the
    # Outreach tab, where you review it and send it manually on LinkedIn.
    if is_first_degree_connection(page, profile_url):
        print(f"[Connect] {name} is 1st-degree — queuing DM draft for manual send.", flush=True)
        try:
    ...
    # 2nd/3rd → empty connection request (still auto-sent). The DM stays a draft;
    # once they accept, the sweeper flips it to "ready to DM" for manual send.
...
def draft_email_node(state: AgentState) -> dict:
    ...
    try:
        outreach.record_draft(
            ...
            post_excerpt=post.get("content") or "",
...
def draft_dm_node(state: AgentState) -> dict:
    ...
    # 1st-degree → already connected; queue the DM as a draft ready to send.
    # The bot never sends DMs — you review it under "Ready to DM" in the
    # Outreach tab and send it manually on LinkedIn.
    if is_first_degree_connection(page, profile_url):
        print(f"[Connect] {author_name} is 1st-degree — queuing DM draft for manual send.", flush=True)
        try:
            pending_db.add_pending(
                profile_url=profile_url,
                name=author_name,
    ...
    # 2nd/3rd → connection request first (still auto-sent). The DM stays a draft;
    # once they accept, the sweeper flips it to "ready to DM" for manual send.
    if not pending_db.can_send_today():
...
def check_pending_connections(headless: bool = True, max_to_dm: int = 20) -> dict:
    """Sweeper sweep — status refresh only.

    The bot never sends DMs anymore: every DM is a draft you send manually on
    LinkedIn. So this sweep does ONE thing — re-check each pending connection
    invite, and when LinkedIn shows a Message button (the invite was accepted),
    flip the entry to 'accepted'. That surfaces the queued DM draft under
    "Ready to DM" in the Outreach tab, where you review and send it yourself."""
    still_pending_items = pending_db.list_items(status="pending")

    if not still_pending_items:
        return {
    ...
    checked = accepted = still_pending = 0
    ...
    # Refresh status of every pending invite. Each check runs in its own tab
    # that gets closed before moving on.
    for it in still_pending_items[:max_to_dm]:
        ...
        if status == "accepted":
            accepted += 1
            pending_db.update_status(profile_url, "accepted", mark_accepted=True)
            print(f"[CheckPending] Accepted: {it.get('name')} — DM draft now ready for you to send manually.", flush=True)
        elif status == "pending":
            still_pending += 1
    ...
    return {
        ...
        "still_ripening": len(pending_db.list_items(status="accepted")),
    }
...
# ─────────────────────────── Capability agents ───────────────────────────
# The system is structured as THREE self-contained capability agents, each
# compiled as its own LangGraph sub-graph, plus a top-level Orchestrator
# (agent/orchestrator.py) that picks which agent(s) to run per the modes the
# user selected. Every agent has the same shape:
#
#     init → <search> → evaluate → <action node(s)> → loop back to <search>
#
# The shared `evaluate` node is the single LLM "brain" (it scores the scraped
# content against the CV and returns an action); the shared `router` drives
# every edge. Splitting per agent means each graph only registers the nodes it
# can actually reach — the Easy Apply agent never routes to `network`, the Post
# agent never routes to `apply`, and so on.

# The one search node that fronts each agent.
_AGENT_SEARCH_NODE = {
    "JOB": ("search_job", search_job_node),
    "PERSON": ("search_person", search_person_node),
    "POST": ("search_post", search_post_node),
}

# The action node(s) reachable from `evaluate` for each agent. These are the
# only destinations evaluate_node can emit for that content type, so the
# evaluate edge map is scoped to exactly this set + the search loop-back + end.
_AGENT_ACTION_NODES = {
    "JOB": {"apply": apply_node},
    "PERSON": {"network": network_node},
    "POST": {
        "draft_email": draft_email_node,
        "draft_dm": draft_dm_node,
        "external_link": external_link_node,
    },
}


def build_agent(search_type: str):
    """Compile the self-contained sub-graph for ONE capability agent.

    `search_type` is JOB / PERSON / POST. The returned graph wires:
        init → <search> → evaluate → <action node(s)> → (loop | end)
    using the shared `router`. Edge maps are scoped to only the nodes this
    agent registers, so the router can never route to a node that isn't here."""
    search_type = (search_type or "JOB").upper()
    if search_type not in _AGENT_SEARCH_NODE:
        search_type = "JOB"
    search_name, search_fn = _AGENT_SEARCH_NODE[search_type]
    action_nodes = _AGENT_ACTION_NODES[search_type]

    workflow = StateGraph(AgentState)
    workflow.add_node("init", init_browser_node)
    workflow.add_node(search_name, search_fn)
    workflow.add_node("evaluate", evaluate_node)
    for name, fn in action_nodes.items():
        workflow.add_node(name, fn)

    workflow.set_entry_point("init")

    # init → search | end
    workflow.add_conditional_edges("init", router, {search_name: search_name, "end": END})
    # search → evaluate | loop (on SEARCHED_EMPTY) | end
    workflow.add_conditional_edges(
        search_name, router, {"evaluate": "evaluate", search_name: search_name, "end": END}
    )
    # evaluate → action node(s) | loop (on SKIP) | end
    eval_map = {name: name for name in action_nodes}
    eval_map[search_name] = search_name
    eval_map["end"] = END
    workflow.add_conditional_edges("evaluate", router, eval_map)
    # each action node → loop | end
    for name in action_nodes:
        workflow.add_conditional_edges(name, router, {search_name: search_name, "end": END})
