...
from agent.graph import check_pending_connections
...
async def lifespan(app: FastAPI):
    ...
    # Pending-connection sweeper: every 15 min, refresh invite statuses. When an
    # invite is accepted, its queued DM draft flips to "ready to DM" in the
    # Outreach tab. The bot never sends DMs — the user reviews and sends each
    # one manually on LinkedIn.
...
def detail_from_node(search_type: str, action: str, value: dict) -> dict | None:
    ...
    if action == "SEARCHED_POST":
        ...
        return {
            ...
            "url": permalink,
            "summary": (details.get("content") or "").strip(),
...
async def run_agent_workflow(config: StartRequest, search_type: str):
    ...
    agent_name = ORCHESTRATOR.name_for(search_type)
    await log_queue.put(_stash({
        "message": (
            f"[{search_type}] {agent_name} initializing - Role: {config.role}, "
    ...
    try:
        ...
        graph_app = ORCHESTRATOR.agent_for(search_type)
                ...
                for output in graph_app.stream(initial_state, {"recursion_limit": 500}):
                    for key, value in output.items():
                        ...
                        if search_detail:
                            ...
                            _eval_kind = evaluated_detail.get("kind")
                            _eval_bucket = (
                                "jobs" if _eval_kind == "Job"
                                else "posts" if _eval_kind == "Post"
                                else "profiles"
                            )
                            record_activity(
                                _eval_bucket,
...
async def run_workflows_sequentially(config: StartRequest, search_types: list[str]):
    # The Orchestrator turns the selected modes into an ordered, de-duped plan
    # of (search_type, agent_name) and we run each capability agent in turn.
    plan = ORCHESTRATOR.plan(search_types)
    if len(plan) > 1:
        await log_queue.put(_stash({
            "message": "Orchestrator plan: " + " → ".join(name for _, name in plan),
            "type": "system",
        }))
    for search_type, _agent_name in plan:
...
async def start_agent(req: StartRequest):
    ...
    async with active_tasks_lock:
        ...
        # Count the de-duped orchestrator plan, not the raw request — the active
        # counter must match how many agents actually run or the "all done"
        # signal never fires.
        plan = ORCHESTRATOR.plan(req.search_types)
        if not plan:
            return {"status": "error", "message": "No valid modes selected"}
...
class ConnectionStatusRequest(BaseModel):
    profile_url: str
    status: str  # pending | accepted | dm_sent | declined | dm_failed


@app.post("/api/outreach/connections/status")
async def api_connection_status(req: ConnectionStatusRequest):
    """Manually set a connection's status. Used by the 'Mark DM sent' button on
    the Outreach tab — the bot drafts DMs but never sends them, so the user
    sends on LinkedIn and then marks the entry sent here."""
    if req.status not in pending_db.VALID_STATUSES:
        return JSONResponse({"error": "invalid_status"}, status_code=400)
    pending_db.update_status(
        req.profile_url, req.status,
        dm_sent=(req.status == "dm_sent"),
    )
    return {"status": "ok"}
